---
title: "Rogue River Migration Timing Diversity Analysis"
output:
  html_document:
    code_folding: hide
    df_print: paged
...
__(1) Full Dataset:__ all 390 GTseq markers available in the full SFGL O mykiss GTseq panel. (The dataset here has fewer markers due to filtering - see genotyping notebook) (353 markers) 
__(2) LD-thinned Full Dataset:__ The full dataset, but thinned to an R^2 of 0.2. (328 markers)
__(3) Migration-timing markers:__ a subset of adaptive markers with specific associations with run-timing in O. mykiss. 
...
```{r HE sig, cache=TRUE, warning=FALSE, message=FALSE}
...
(1) We're going to have a lot of similar seeming analyses on slightly different datasets that ask fundamentally different questions. Let's make this clear by organizing not into analysis type but by the questions we are addressing with each (e.g. _de novo_ vs _a priori_)   
...
## _De Novo_ Genetic Structure

In this section we ask what structure exists within the dataset, then examine how the inferred genetic structure maps onto the _a priori_ defined phenotypic groups.
...
a <- ggplot(data = pca_vals, aes(Axis1, Axis2, color = run))+geom_point(alpha = 0.5)+stat_ellipse()+theme_classic()+scale_color_viridis_d(name = "")+xlab("Principal Component 1")+ylab("Principal Component 2")

eig <- as.data.frame(bind_cols(pca1$eig[1:10], seq(1, 10)))
...
__burnin/mcmc:__ ran with k=1-6 for 100k iteration to check for convergence of alpha, strong convergence after a few hundred burn-in iterations, used 10,000/20,000 for final runs   
...
Let's also summarise the mean and variance of individual ancestry proportions amogn phenotypic groups.
...
(2) at K=3 and greater, there is a clear summer associated genetic cluster and awinter associated genetic cluster with highly variable amounts of introgression of these genetic clusters into fall and half pounder assigned fish.  
...
(4) There is also a genetic cluster that never reaches high ancestry proportion in early-summer and winter, but is hgihlighy variable among late-summer run fish
...
Some notes here:
(1) This is a de novo DAPC, even though it relies on detection of distinct genetic clusters within the dataset, it does not assume that the phenotypic groups are distinct genetic lineages. The phenotypes of individuals is not included in the DAPC in any way. 
() We do color the points of the figures afterwards to see how well the genetic clusters correlate to phentoypes.
...
```{r, message=FALSE, warning=FALSE, fig.cap="BIC for k-means clustering across different numbers of genetic clusters"}
#k means clustering, keep a lot (350 pcs) (kmeans won't overfit with two many pcs)
...
The model fit is best at k=3 according to BIC. BICof k-means clustering tends to decrease until best fit only in a perfect island model, in practice, best K is often found at the lowest K that is a substantial improvement from the last (in this case 2). Here we use k=3, but we should also consider the k = 2 results.
...
The optim.a.score approach found very different optimal n.pcs from run to run. This seems to be because a.score increases up to about 3 pcs and then has a long plateau. Different values are chosen as the "best" along this plateau from run to run. We chose 3 pcs, but there was littel qualitative difference in the DAPC results using any number of PCs in or near the a.score plateau 3 - 100 pcs.

```{r, message=FALSE, warning=FALSE}
#first optimize the PCs retained based on the a.score, so run dapc on the full pcs
#invisible(dapc_full_denovo <- dapc(unlinked_genind, clusts3$grp, n.pca = 330, n.da = 2 ))
...
# dapc_full_denovo <- dapc(unlinked_genind, clusts3$grp, n.pca = 100, n.da = 2 )
#optim_a_denovo <- optim.a.score(dapc_full_denovo, n = 50, smart = FALSE)
#nice now we use the optimized a score to run our dapc for real
clusts3 <- find.clusters(unlinked_genind, n.pca = 330, n.clust = 3)
dapc_full_denovo <- dapc(unlinked_genind, clusts3$grp, n.pca = 3, n.da = 2 )
...
#pub figure
a <- ggplot(data=plot_data)+geom_point(aes(x=LD1, y=LD2, color=pop), alpha = 0.6)+scale_color_viridis_d(labels = c("Late-Summer", "Half-Pounder", "Early-Summer", "Winter"))+theme_classic(base_size = 8)+guides(color=guide_legend("Run-Type"))+stat_ellipse(aes(x=LD1, y=LD2, color = pop))

b <- ggplot(data=plot_data)+geom_point(aes(x=LD1, y=LD2, color=grp), alpha = 0.6)+scale_color_viridis_d()+theme_classic(base_size = 8)+guides(color=guide_legend("K-means cluster"))+stat_ellipse(aes(x=LD1, y=LD2, color = grp))

plot_grid(b, a, ncol = 1, labels = "auto")

#ideal ratio for 84mm figure (single column) 3.31  x 4.2

# #make into publication table
# loading_table1 <- as.data.frame(marker_loadings1$var.values)
# colnames(loading_table1) <- "loading"
# loading_table1 %<>%
#   rownames_to_column(var = "marker") %>%
#   mutate(marker = substr(marker,1,nchar(marker)-2))
# 
# loading_table2 <- as.data.frame(marker_loadings2$var.values)
# colnames(loading_table2) <- "loading"
# loading_table2 %<>%
#   rownames_to_column(var = "marker") %>%
#   mutate(marker = substr(marker,1,nchar(marker)-2))
# 
# 
# loading_table <- bind_rows(loading_table1, loading_table2, .id = "axis")
# rm(loading_table1)
# rm(loading_table2)
# 
# write_tsv(loading_table %>%
#  left_join(select(marker_mapping2, marker, chr, "Panel Origin", "Presumed Type")) %>%
#  distinct(marker, axis, .keep_all = TRUE), file = "loading.txt")
...
## _A priori_ Genetic Structure

In the _de novo_ section above, we query the dataset to identify patterns without considering the phenotypic groups, then examine how well this structure maps onto our phenotypic groups. In this section we change the inference. We ask what differences exist between a priori assigned phenotypic classes. 
...
marker_loadings1 <- loadingplot(dapc_full_apriori_unl$var.contr, axis=1,thres=.02, lab.jitter=1, main = "loading plot for DA 1")
markers1 <- unique(substr(names(marker_loadings1$var.values),1,nchar(names(marker_loadings1$var.values))-2))

marker_loadings2 <- loadingplot(dapc_full_apriori_unl$var.contr, axis=2,thres=.008, lab.jitter=1, main = "loading plot for DA 2")
markers2 <- unique(substr(names(marker_loadings2$var.values),1,nchar(names(marker_loadings2$var.values))-2))
...
#for ms figures
plot_data %<>%
  #mutate(pop = case_when(pop == "halfpounder" ~ "Half-Pounder",
   #                      pop == "Summer" ~ "Early-Summer",
  #                       pop == "fall" ~ "Late-Summer",
  #                       TRUE ~ pop)) %>%
  mutate(pop = as.factor(plot_data$pop)) %>%
 mutate(pop = fct_relevel(pop, "fall", "halfpounder", "Summer", "Winter"))
ggplot(data=plot_data)+geom_point(aes(x=LD1, y=LD2, color = pop), alpha = 0.5, size = 2)+theme_classic(base_size = 10)+scale_color_viridis_d(name = "Run",labels = c("Late-Summer", "Half-Pounder", "Early-Summer", "Winter" ))+stat_ellipse(aes(x=LD1, y=LD2, color = pop)) # 6.85 x 4 in
...
Just as with the other _a priori_ DAPC, we try to be extra cautious about overfitting and examined best number PCAs to retain using two different cross validation approaches and the optim.a.score function from the adegenet package developers.
...
We build a genetic axis that discriminates between our samples of early summer and winter runs, then examine where half-pounders and late-summers lie along this axis and attempt to classify them into categories based on 99% credible intervals around known early-summer adn winter individuals.

> Note: we did this two ways in a pilot analysis, in the first we run a true classification scheme: exclude the fall and half pounder fish, create a classifier using winter and summer samples only, then predict the fall and half pounder membership based on this classifier. 
However if fall run fish represent are a real group, the classifier should include all three or four groups and therefore would default back to the original DAPC. As we have already seen from the PCA, STRUCTURE and DAPC results so far, there is no evidence that late-summer run fish represent a distinct genetic lineage, so this approach (use the a priori DAPC on all four groups) is not valid. I only present the true classifier here and in the manuscript. In any case, the effect on the end result is small, qualitatively the results were similar because the first DA in the a priori DAPC is largely driven by migration timing markers anyway
...
"Our LD-pruned dataset excluded 23 highly linked markers (r2 > 0.2), including seven of the 12 migration timing markers, four of the five residency vs. anadromy associated markers and three neutral markers physically close to the residency vs. anadromy associated markers. Investigating the pattern of LD among the migration timing markers (Supplemental Figure 7) reveals that there are two blocks in this region in strong LD, with SNPs on the border of these blocks demonstrating minor LD with any other SNP. Therefore the five migration timing markers retained after LD filtering include one SNP from each high LD block, a SNP physically in between these blocks and two SNPs on the border of the entire genomic region."
...
ggplot(pred.mm) + 
   ...
   theme_classic()+geom_rect(aes(xmin = 3.56, xmax=7.51, ymin = -Inf, ymax = Inf), fill = "#35B77980", alpha = 0.05)+geom_rect(aes(xmin = -8, xmax=-4.46, ymin = -Inf, ymax = Inf), fill = "#FDE72580", alpha = 0.05)+ scale_fill_manual(values = c("white", "black"), name = "Year")+scale_shape(solid = FALSE)+ theme(text = element_text(size = 10))
...
Let's also present an alternative figure that presents the data in a way that is easy to understand (time along x axis, right is later)

```{r}

fall_mixed <- lme4::lmer(LD1 ~ jdate + (1|year.x), data = filter(cor_data, run =="fall"))
fall_reduced <- lme4::lmer(LD1 ~ 1 + (1|year.x), data = filter(cor_data, run =="fall"))

anova(fall_reduced, fall_mixed)

summary(fall_mixed)

ggplot(data = filter(cor_data, run =="fall"))+geom_point(aes( jdate, LD1, color = year.x))+geom_smooth(aes( jdate, LD1, color = year.x), method = "lm")+ggtitle("plot highlighting year effect")
...
require(ggeffects)
# Extract the prediction data frame
pred.mm <- ggpredict(fall_mixed, terms = c("jdate"))  # this gives overall predictions for the model
...
ggplot(pred.mm) + 
   ...
   geom_ribbon(aes(x = x, ymin = conf.low, ymax = conf.high), 
               fill = "lightgrey", alpha = 0.5) +  # error band
   geom_point(data = filter(cor_data, run =="fall"),                      # adding the raw data (scaled values)
              aes(y = LD1, x = jdate, fill = year.x), shape = 21) + 
   labs(y = "LD1 score\n(negative is more winter-like)", x = "Julian Day of Sampling") + 
   theme_classic()+geom_rect(aes(ymin = 3.56, ymax=7.51, xmin = -Inf, xmax = Inf), fill = "#35B77980", alpha = 0.05)+geom_rect(aes(ymin = -8, ymax=-4.46, xmin = -Inf, xmax = Inf), fill = "#FDE72580" , alpha = 0.05)+ scale_fill_manual(values = c("white", "black"), name = "Year")+scale_shape(solid = FALSE)
