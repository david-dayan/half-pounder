...
ggplot(ldr)+geom_tile(aes(snp1, snp2, fill=rbarD))+theme_classic()+theme(axis.text.x = element_text(angle = 90))+scale_fill_gradient(low = "#FFFFFF", high= "#2c3e50", limits = c(0,1))

ldr_f <- ldr_f %>%
  filter(!(snp1 %in%  uninf_markers)) %>%
  filter(!(snp2 %in%  uninf_markers))

ggplot(ldr_f)+geom_tile(aes(snp1, snp2, fill=rbarD))+theme_classic()+theme(axis.text.x = element_text(angle = 90))+scale_fill_gradient(low = "#FFFFFF", high= "#2c3e50", limits = c(0,1))
